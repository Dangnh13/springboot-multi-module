...
public class Dates {
    ...
    public static String now(String pattern) {
        DateTimeFormatter dtFormatterOld = DateTimeFormatter.ofPattern(CommonConstant.DATE_FORMAT.YYYY_MM_DD_HH_MM_SS_DASH);
        DateTimeFormatter dtFormatterNew = DateTimeFormatter.ofPattern(pattern);
        String now = LocalDateTime.now().format(dtFormatterOld);
        LocalDateTime datetime = LocalDateTime.parse(now, dtFormatterOld);
        return datetime.format(dtFormatterNew);
    }

    public static String getFirstDate(String dateStr) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern(CommonConstant.DATE_FORMAT.YYYYMMDD);
        LocalDate dateTime = LocalDate.parse(dateStr, formatter);
        return dateTime.withDayOfMonth(1).format(formatter);
    }
}
